...
export function CashStation({ operatorName, onLogout }: CashStationProps) {
  ...
  const paymentMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Log the action via WebSocket - this is critical for logs to work
      socketProvider.send({
        type: 'NEW_PAYMENT',
        data
      });
              ...
              <div className="space-y-4 text-sm text-neutral-600">
                <div className="flex items-start space-x-3">
                  <div className="flex-shrink-0 h-6 w-6 rounded-full bg-blue-100 flex items-center justify-center">
                    <span className="material-icons text-xs text-blue-600">lightbulb</span>
                  </div>
                  <div>
                    <p className="font-medium text-neutral-700">Verify ID</p>
                    <p>Always check the student's ID before processing payment.</p>
                  </div>
                </div>
                <div className="flex items-start space-x-3">
                  <div className="flex-shrink-0 h-6 w-6 rounded-full bg-blue-100 flex items-center justify-center">
                    <span className="material-icons text-xs text-blue-600">attach_money</span>
                  </div>
                  <div>
                    <p className="font-medium text-neutral-700">Count Twice</p>
                    <p>Count money received and given as change at least twice.</p>
                  </div>
                </div>
                <div className="flex items-start space-x-3">
                  <div className="flex-shrink-0 h-6 w-6 rounded-full bg-blue-100 flex items-center justify-center">
                    <span className="material-icons text-xs text-blue-600">receipt_long</span>
                  </div>
                  <div>
                    <p className="font-medium text-neutral-700">Receipt</p>
                    <p>Direct students to keep receipt until book is received.</p>
                  </div>
                </div>
              </div>
            </CardContent>
